...
public class BinarySearchTree<E extends Comparable,T> implements BinaryTreeInterface<T> {
	...
	// Search the element
	public void searchRec(Node p, E x) {
		if( p == null) {
			System.out.println(x + "Not found!");
		}else if(p.getElement() == x) {
			System.out.println(x + "Founded!");
		}else if(x.compareTo(p.getElement()) < 0) {
			searchRec(p.getLeft(), x);
		}else {
			searchRec(p.getRight(), x);
		}
	}
	...
	// Insert a element
	public void insertRec(E x, Node p) {
		if(x.compareTo(p.getElement()) < 0) {
			if(p.getLeft() != null) {
				insertRec(x, p.getLeft());
			}else {
				p.left = new Node<E>(x, p, null, null);
			}
		}
		if(x.compareTo(p.getElement()) > 0) {
			if(p.getRight() != null) {
				insertRec(x, p.getRight());
			}else {
				p.right = new Node<E>(x, p, null, null);
			}
		}
	}
	...
	// Delete a element
	public void deleteRec(E x, Node p) {
		if(p.getLeft() == null && p.getRight() == null) {
			...
			if(p.getElement() == x) {
				...
				deleteRec(x, p.getRight());
			}
		}else if( p.getRight() == null) {
			if(p.getElement() == x) {
				p = p.getLeft();
			}else
				deleteRec(x, p.getLeft());
		}else if(p.getLeft()!=null && p.getRight()!=null) {
			if(p.getElement() == x) {
				findMin(p.getRight());
				deleteRec((E) p.getElement(), p.getRight());
			}else if(x.compareTo(p.getElement()) < 0) {
				deleteRec(x, p.getLeft());
			}else {
				deleteRec(x, p.getRight());
